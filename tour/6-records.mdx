...
<InstructionsWrapper>
	<div className="instructions-main">
		...
		To retrive a value from a record, use square brackets with the key name (e.g., `x["name"]`). A `null` value is returned if the field
		...
		To assign a value to a record, use square brackets with the key name (e.g., `x["name"]`). If the field does not exist on the record, an `invalid field`
		...
		To assign fields to a record, place the field between `<` and `>` characters:
		...
		When your record contains other record, you use multiple indexes to retrieve and assign values:
